...
use bevy::prelude::{ResMut, Assets, Color, Vec2, shape, Mesh, Transform, Camera2dBundle, info, default, Query, Component, Entity, Vec3, EventReader, ClearColor};
...
pub fn setup_game(
    ...
    let size_data = SizeDate::new(game_data.map.width, game_data.map.height, window.width(), window.height());
    init_world(&mut commands,  &mut materials, &mut meshes, &mut game_data, &size_data);
...
pub fn spawn_quad(
    ...
    let id = commands.spawn(MaterialMesh2dBundle {
        ...
        transform: Transform::from_xyz(quad_x, quad_y, 0.).with_scale(Vec3::new(size_date.quad_width, size_date.quad_height, 0.)),
...
fn refresh_size_date(
    size_date: &mut SizeDate,
    width: f32,
    height: f32,
) {
